...
func SendEmail(userEmail, userName, hadiahORTokenORKeterangannota, typeSend, imagePath string) error {
	...
	if typeSend == "AnnouncementGift" {
		...
		body = "Dear " + userName + "\r\n" +
			"Please click http://localhost:8080/forgot/password/reset?token=" + hadiahORTokenORKeterangannota + " to reset your password."
...
// SendEmailForgotPassword sends an email with a password reset link
// func SendEmailForgotPassword(email, username, token string) error {
// 	// SMTP authentication
// 	auth := smtp.PlainAuth(
// 		"",
// 		"cabangbanyak@gmail.com",
